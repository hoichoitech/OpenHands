require("../_virtual/_rolldown/runtime.cjs");const e=require("../constants/child-conversation.cjs");var t=`Start a NEW, independent conversation to work on a well-scoped task in parallel with this one, without the user having to run any CLI command. The new conversation is recorded as a child of this one.
...
with "${e.CHILD_CONVERSATION_RESULT_PREFIX.trim()}" that carries the child's
...
reports an error, fix the parameters and call this tool again.`,n={name:e.LAUNCH_CHILD_CONVERSATION_TOOL_NAME,description:t,parameters:{type:`object`,additionalProperties:!1,properties:{target:{type:`string`,enum:[...e.CHILD_CONVERSATION_TARGETS],description:`Where the child runs. 'local' reuses this machine and this conversation's workspace; 'cloud' runs in an isolated OpenHands Cloud sandbox.`},task:{type:`string`,description:`Self-contained task brief, sent as the child's first message. The child cannot see this conversation, so state the goal, constraints, expected output and handoff criteria.`},title:{type:`string`,description:`Optional short title for the child conversation. Omit to let it title itself from the task.`},repository:{type:`string`,description:`target='cloud' only. Repository to clone into the sandbox, as 'owner/repo'. Defaults to this conversation's repository when it has one.`},branch:{type:`string`,description:`target='cloud' only, and only together with 'repository'. Defaults to the repository's default branch.`},isolation:{type:`string`,enum:[...e.CHILD_CONVERSATION_ISOLATIONS],description:`target='local' only. 'worktree' (default) gives the child its own git worktree and branch; 'shared' runs it in this conversation's exact directory.`}},required:[`target`,`task`]},annotations:{readOnlyHint:!1,destructiveHint:!1,idempotentHint:!1,openWorldHint:!0}};exports.LAUNCH_CHILD_CONVERSATION_CLIENT_TOOL=n;
//# sourceMappingURL=launch-child-conversation-client-tool.cjs.map